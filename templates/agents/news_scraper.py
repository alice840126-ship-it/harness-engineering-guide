...
def _wrap_if_needed(text: Optional[str], shield: bool, source: Optional[str]) -> Optional[str]:
    """shield=True일 때만 injection_shield.wrap_external로 감싸기 (실패/None이면 원본 반환)"""
    if not shield or not text:
        return text
    try:
        import sys as _sys
        ...
        return wrap_external(text, source=source or "news")
    except Exception:
        return text
...
class NewsScraper(BaseAgent):
    ...
    def _scrape_single(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fetch_full and NEWSPAPER_AVAILABLE:
            for item in news_items:
                link = item.get('link', '')
                if link:
                    full_text = self.fetch_full_article(link, shield=shield, shield_source=link)
                    if full_text:
    ...
    def fetch_full_article(self, url: str, max_sentences: int = 3,
                           shield: bool = False, shield_source: Optional[str] = None) -> Optional[str]:
        """기사 본문 전체 추출
        ...
        try:
            ...
            return _wrap_if_needed(summary, shield, shield_source or url)
